...
################# Flatten LEVEL 1 ####################
dat1 <- markdata %>% filter(., workflow_id == 25161)
...
data_out$tool_label <- ifelse(data_out$toolIndex ==1, "People",
                               ifelse(data_out$toolIndex ==0, "Fish","Other"))
...
##### agreement among users; the key idea is to evaluate how consistently users count the same number of fish/people of a given species within each photo.
...
clustered_data %>%
  ...
  coord_fixed() +
  labs(
    title = "Agreement Heatmap for Photo 41136326",
...
clustered_data %>%
  ...
  coord_fixed() +
  labs(
    title = "Heatmap + Clusters: Photo 41136326",
